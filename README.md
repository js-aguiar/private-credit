...
alarms, an SNS topic for alerts, and a **document catalog** (private S3 + CloudFront
HTTPS + API Gateway HTTP API + VPC Lambda). **No scraper schedule is created** —
invoke the scraper Lambdas manually (or wire your own trigger later).
